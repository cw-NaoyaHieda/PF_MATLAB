---
title: "PF_MATLABとKalmanの比較 パーティクル100個"
...
頑張ったけど、最適化計算がやはり遅いので、ちょっとパーティクル数を減らしてフィルタリングしてみる。
...
rho <- 0.08
X_0 <- -2.5
answer_X <- read.csv("data/X_100.csv",header = F)
data <- read.csv("data/DR_100.csv",header = F)
colnames(answer_X) <- c("answer")
pf <- read.csv("data/matlab_pf_100.csv",header = F)
colnames(pf) <- c("Particle_filter")
sm <- read.csv("data/matlab_sm_100.csv",header = F)
