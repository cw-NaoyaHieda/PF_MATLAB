...
for count2 = 1:5
  ...
  params_opter((count2-1) * 31 + 1,:) = [count2, 0, first_pm,0];
  ...
  for count = 1:30
    ...
    PMCEM(first_pm)
    fval
    params_opter((count2-1) * 31 + count + 1,:) = [count2,count,params,fval];
...
csvwrite('data_rep/parameter_mini.csv',params_opter)
plot(1:dT,X)
hold on
